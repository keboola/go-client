...
func runGZUploadTest(t *testing.T, c client.Sender, f *storageapi.File) {
	t.Helper()
	ctx := context.Background()
	...
	res, err := fileTest.GetUploadedFile(t, file)
	if err != nil {
		assert.Fail(t, "getting file %s failed: %w", file.Url, err)
	}
	res = fileTest.Decompress(t, res)
	assert.Equal(t, "col1,col2\nval1,val2\n", res)
}
