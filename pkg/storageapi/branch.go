...
func CreateBranchRequest(branch *Branch) client.APIRequest[*Branch] {
	job, request := createBranchRequest(branch)
...
func createBranchRequest(branch *Branch) (*Job, client.HTTPRequest) {
	// ID is autogenerated
	if branch.ID != 0 {
...
func DeleteBranchRequest(key BranchKey) client.APIRequest[client.NoResult] {
	request := deleteBranchRequest(key).
...
func deleteBranchRequest(key BranchKey) client.APIRequest[*Job] {
	result := &Job{}
	request := newRequest().
		WithResult(result).
