...
func LogTracer(wr io.Writer) Factory {
	...
	return func(ctx context.Context, reqDef request.HTTPRequest) (context.Context, *ClientTrace) {
		...
		t.HTTPRequestDone = func(r *http.Response, read int64, err error) {
			doneTime = time.Now()
			var errorStr string
			if err == nil {
			...
			t.log(requestID, fmt.Sprintf(`DONE  %s "%s" | %d | %vB | %s%s`, req.Method, req.URL.String(), statusCode, read, doneTime.Sub(startTime).String(), errorStr))
		}
